...
/***** 输出 if 语句的条件的三地址代码 *****/
void condition(int L1,int L2) //L1,L2 分别为 if 条件为 true 和 false 时候的跳转地址
{
 string result=link("t",tID++);
...
void yuyifenxi(int next,int &flag) 
{
 ...
 else if(shuru[ip]=="while")      //while 的语义子程序
 {
   ip++;
   int L1=LID++;
   int L2=LID++;
